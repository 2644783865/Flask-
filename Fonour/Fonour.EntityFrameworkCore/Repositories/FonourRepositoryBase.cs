...
namespace Fonour.EntityFrameworkCore.Repositories
{
    ...
    public abstract class FonourRepositoryBase<TEntity, TPrimaryKey> : IRepository<TEntity, TPrimaryKey> where TEntity : Entity<TPrimaryKey>
    {
        ...
        public List<TEntity> GetAllList2(Expression<Func< TEntity, bool>> predicate,string Id)
        {
         
                List<TEntity> dataSource = null;
                if (predicate.ToString() != "00000000-0000-0000-0000-000000000000")
                {
                    string sql = string.Format(@"select m.*
                        from Users u
                        inner
                        join UserRoles ur on u.Id = ur.UserId
                        inner join Roles rs on rs.Id = ur.RoleId
                        inner join RoleMenus rm on rm.RoleId = rs.Id
                        inner join Menus m on m.Id = rm.MenuId
                        where u.Id = '{0}'", Id);

                    var iQueryTable = _dbContext.Set<TEntity>().
                          FromSql(sql);
                    dataSource = iQueryTable.ToList();
                }
                else
                {
                    dataSource = _dbContext.Set<TEntity>().Where(predicate).ToList();
                }
                return dataSource;
            }
        /// <summary>
